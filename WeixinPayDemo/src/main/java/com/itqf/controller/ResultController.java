...
import com.itqf.utils.PayConfigUtil;
import com.itqf.utils.XMLUtil;
 ...
 * 处理支付结果的 Servlet
 * 用于响应微信支付结果的 servlet,这个工具类可能有问题
 */
public class ResultController extends HttpServlet {
    ...
    public void weixin_notify(HttpServletRequest request,HttpServletResponse
        ...
        FileOutputStream fileOutputStream = new
                FileOutputStream(path+"/result.txt", true);//创建输出流,写入结果用,实际开发中删除由此到上面的内容
       //读取参数
...
//通知微信.异步确认成功.必写.不然会一直通知后台.八次之后就认为交易失败了.
                ...
                System.err.println("订单"+out_trade_no+"支付失败,错误信息："
                        + packageParams.get("err_code")+ packageParams.get("err_code"));
